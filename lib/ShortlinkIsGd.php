...
class ShortlinkIsGd implements Shortlink
{
	...
	protected static function apiHelper( $action, $url, $desired = false )
	{
		...
		if( $haveWeHitTheLimit ) return new WP_Error( 'is-gd-error-code-' . $haveWeHitTheLimit->errorcode, $haveWeHitTheLimit->errormessage );
	...
	protected static function getResponse( $url )
	{
		...
		elseif( empty( $request['response'] ) || 200 != $request['response'] ) return new WP_Error( 'response-code-not-valid', 'The response code was not valid.' );
		$response = json_decode( $request['body'] );
		if( !is_object( $response ) ) return new WP_Error( 'response-data-corrupted', 'The response data was corrupted and could not be decoded' );
		return $response;
	}
	...
	protected static function formatRequestURL( $action, $url, $desired = false )
	{
		...
		if( $action == 'shorten' )
		{
			...
			if( $desired && strlen( (string)$desired ) >= 5 )
			{
				...
				if( strlen( $desired >= 5 ) ) $request_arguments['shorturl'] = $desired;
			}
		} else
		{
